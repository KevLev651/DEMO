...
def _region_header_signature(region: Region) -> str:
    header_rows = region.rows[: min(2, len(region.rows))]
    if header_rows:
        header_cells = []
        for row in header_rows:
            header_cells.extend(_normalized_cell(cell) for cell in row.cells[:8] if _normalized_cell(cell))
        if header_cells:
            return " ".join(header_cells)
    return normalize_text(region.text[:240])


def _region_identity_set(region: Region) -> set[str]:
    anchors: set[str] = set()
    for row in region.rows:
        anchors.update(anchor for anchor in _row_identity_candidates(row, region) if len(anchor) >= 2)
    return anchors


def _region_identity_similarity(before_region: Region, after_region: Region) -> float:
    before_ids = _region_identity_set(before_region)
    after_ids = _region_identity_set(after_region)
    if not before_ids or not after_ids:
        return 0.0
    return len(before_ids & after_ids) / max(len(before_ids | after_ids), 1)
...
def _match_region_score(
    ...
    header_similarity = SequenceMatcher(
        None,
        _region_header_signature(before_region),
        _region_header_signature(after_region),
    ).ratio()
    identity_similarity = 0.0
    if before_region.kind in {"bom-like", "table-like"} and after_region.kind in {"bom-like", "table-like"}:
        identity_similarity = _region_identity_similarity(before_region, after_region)
    row_similarity = 0.0
    ...
    if before_region.kind == "bom-like":
        return (identity_similarity * 0.42) + (header_similarity * 0.23) + (overlap * 0.20) + (text_similarity * 0.10) + (row_similarity * 0.05)
    if before_region.kind == "table-like":
        return (identity_similarity * 0.24) + (header_similarity * 0.18) + (overlap * 0.33) + (text_similarity * 0.18) + (row_similarity * 0.07)
...
def _header_text(cell: str) -> str:
    return " ".join(str(cell or "").strip().lower().replace("_", " ").split())


def _header_looks_like_id(cell: str) -> bool:
    header = _header_text(cell)
    if not header:
        return False
    if header in ID_HEADER_TERMS:
        return True
    return header.endswith(" id") or header.endswith(" no") or header.endswith(" number")


def _cell_looks_like_id(cell: str) -> bool:
    ...
    if has_alpha and has_digit:
        return True
    return bool(re.fullmatch(r"\d{3,}", normalized))


def _embedded_id_tokens(value: str) -> list[str]:
    tokens: list[str] = []
    for match in ROW_ID_TOKEN_RE.finditer(value):
        token = _normalized_cell(match.group(0))
        if len(token) < 4:
            continue
        if token.startswith("rev"):
            continue
        if token.startswith("data") and token[4:].isdigit():
            continue
...
def _row_identity_candidates(row, region: Region) -> list[str]:
    candidates: list[str] = []
    rows = list(getattr(region, "rows", []) or [])
    header_row = rows[0] if rows and len(rows) > 1 else None
    if header_row is not None and row is not header_row:
        for index, cell in enumerate(row.cells):
            if index >= len(header_row.cells):
                continue
            if _header_looks_like_id(header_row.cells[index]) and _cell_looks_like_id(cell):
                normalized = _normalized_cell(cell)
                if normalized and normalized not in candidates:
                    candidates.append(normalized)
    for token in _embedded_id_tokens(getattr(row, "text", "") or ""):
        if token not in candidates:
            candidates.append(token)
    for cell in row.cells[:6]:
        for token in _embedded_id_tokens(cell):
            if token not in candidates:
                candidates.append(token)
    ...
    for cell in row.cells[:4]:
        normalized = _normalized_cell(cell)
        if _cell_looks_like_id(cell) and normalized not in candidates:
            candidates.append(normalized)
    return candidates


def _same_row_content(before_row, after_row) -> bool:
    if before_row.normalized_text == after_row.normalized_text:
        return True
    before_cells = [_normalized_cell(cell) for cell in before_row.cells if _normalized_cell(cell)]
    after_cells = [_normalized_cell(cell) for cell in after_row.cells if _normalized_cell(cell)]
    return bool(before_cells) and before_cells == after_cells
...
def _moved_row_diff(before_row, after_row, page_index: int, region_id: str, region_kind: str, confidence: float = 0.90) -> RowDiff:
    return RowDiff(
        page_index=page_index,
        region_id=region_id,
        region_kind=region_kind,
...
def compare_rows(before_region: Region, after_region: Region, page_index: int) -> list[RowDiff]:
    ...
    for row in before_rows:
        for anchor in _row_identity_candidates(row, before_region):
            if anchor in before_anchor_map:
                duplicate_anchors.add(anchor)
            before_anchor_map[anchor] = row
    for row in after_rows:
        for anchor in _row_identity_candidates(row, after_region):
    ...
    if common_anchors and (len(common_anchors) >= 2 or min(len(before_rows), len(after_rows)) <= 3):
        ...
        for before_row in before_rows:
            anchors = [anchor for anchor in _row_identity_candidates(before_row, before_region) if anchor in common_anchors]
            if not anchors:
                continue
            after_row = after_anchor_map[anchors[0]]
            used_before.add(before_row.row_index)
            used_after.add(after_row.row_index)
            if _same_row_content(before_row, after_row):
                if before_row.row_index != after_row.row_index:
                    row_diffs.append(
                        _moved_row_diff(
                            before_row,
                            after_row,
        ...
        unmatched_before = [before_row for before_row in before_rows if before_row.row_index not in used_before]
        unmatched_after = [after_row for after_row in after_rows if after_row.row_index not in used_after]
        moved_before_indices: set[int] = set()
        moved_after_indices: set[int] = set()
        after_by_text: dict[str, list[object]] = {}
        for after_row in unmatched_after:
            after_by_text.setdefault(after_row.normalized_text, []).append(after_row)
        for before_row in unmatched_before:
            candidates = after_by_text.get(before_row.normalized_text, [])
            if not candidates:
                continue
            after_row = next(
                (
                    ...
                    if candidate.row_index not in moved_after_indices and _same_row_content(before_row, candidate)
                ),
                None,
            )
            if after_row is None:
                continue
            moved_before_indices.add(before_row.row_index)
            moved_after_indices.add(after_row.row_index)
            row_diffs.append(
                _moved_row_diff(
                    before_row,
                    after_row,
        ...
        for before_row in before_rows:
            if before_row.row_index not in used_before:
                if before_row.row_index in moved_before_indices:
                    continue
        ...
        for after_row in after_rows:
            if after_row.row_index not in used_after:
                if after_row.row_index in moved_after_indices:
                    continue
...
def _outline_for_kind(region_kind: str, change_type: str) -> tuple[int, int, int, int]:
    if change_type == "added":
        return (0, 188, 212, 255)  # aqua blue for added items
    if change_type == "removed":
        return (34, 160, 90, 255)  # green for removed items
    if change_type == "moved":
        return (230, 97, 181, 255)  # pink for moved-only items
    return (220, 50, 50, 255)  # red for modified/changed items
...
def _fill_rgba_for_change_type(change_type: str) -> tuple[int, int, int, int]:
    ...
    if change_type == "moved":
        return (248, 199, 231, 72)  # light pink for moved-only items
    return (255, 200, 200, 60)  # light red for modifications/changes
...
def _draw_vector_markers(page: fitz.Page, annotations: list[HighlightAnnotation]) -> None:
    ...
    stroke_pink = (0.90, 0.38, 0.71)
    fill_pink = (0.97, 0.78, 0.90)
    ...
    pink_text_shape = page.new_shape()
    pink_broad_shape = page.new_shape()
    red_text_shape = page.new_shape()
    red_broad_shape = page.new_shape()
    has_bt = has_bb = has_gt = has_gb = has_pt = has_pb = has_rt = has_rb = False
    for annotation in annotations:
        ...
        if annotation.change_type == "added":
            ...
            if is_broad:
                ...
                has_gt = True
        elif annotation.change_type == "moved":
            if is_broad:
                pink_broad_shape.draw_rect(rect)
                has_pb = True
            else:
                pink_text_shape.draw_rect(rect)
                has_pt = True
        else:
            if is_broad:
    ...
    if has_pt:
        pink_text_shape.finish(width=0.0, color=stroke_pink, fill=fill_pink, stroke_opacity=0.0, fill_opacity=0.28)
        pink_text_shape.commit(overlay=True)
    if has_pb:
        pink_broad_shape.finish(width=0.0, color=stroke_pink, fill=fill_pink, stroke_opacity=0.0, fill_opacity=0.10)
        pink_broad_shape.commit(overlay=True)
    if has_rt:
