...
LEADING_ROW_ID_RE = re.compile(r"^\s*(?:\d+\b|[a-z]{1,6}-\d)", re.IGNORECASE)
ROW_ID_TOKEN_RE = re.compile(r"\b(?=[a-z0-9-]*[a-z])(?=[a-z0-9-]*\d)[a-z0-9]+(?:-[a-z0-9]+){1,5}\b", re.IGNORECASE)
NOTES_HEADER_RE = re.compile(r"^\s*notes?\b", re.IGNORECASE)
...
def _rows_in_bbox(words: list[tuple], bbox: BBox) -> list[StructuredRow]:
    ...
    return _merge_continuation_rows(structured)


def _row_has_embedded_id(text: str) -> bool:
    return bool(ROW_ID_TOKEN_RE.search(text))


def _should_merge_continuation(previous: StructuredRow, current: StructuredRow) -> bool:
    current_text = current.text.strip()
    if not current_text:
        return False
    if current_text.startswith("("):
        return _row_has_embedded_id(previous.text)
    if LEADING_ROW_ID_RE.match(current_text):
        return False
    if _row_has_embedded_id(current_text):
        return False
    short_fragment = len(current_text.split()) <= 6 or len(current_text) <= 36
    horizontally_aligned = abs(current.bbox[0] - previous.bbox[0]) <= 36
    vertically_touching = 0.0 <= (current.bbox[1] - previous.bbox[3]) <= 18.0
    return short_fragment and horizontally_aligned and vertically_touching and _row_has_embedded_id(previous.text)


def _merge_continuation_rows(rows: list[StructuredRow]) -> list[StructuredRow]:
    if not rows:
        return []
    merged: list[StructuredRow] = [rows[0]]
    for row in rows[1:]:
        previous = merged[-1]
        if not _should_merge_continuation(previous, row):
            merged.append(row)
            continue
        merged_cells = [*previous.cells]
        if row.cells:
            merged_cells.extend(row.cells)
        else:
            merged_cells.append(row.text)
        merged_boxes = [*previous.cell_bboxes, *row.cell_bboxes] if row.cell_bboxes else [*previous.cell_bboxes, row.bbox]
        merged_text = " ".join(part for part in [previous.text, row.text] if part).strip()
        key_seed = "|".join(merged_cells[:3]) if merged_cells else normalize_text(merged_text)
        merged[-1] = StructuredRow(
            row_index=previous.row_index,
            bbox=bbox_union([previous.bbox, row.bbox]),
            cells=merged_cells,
            text=merged_text,
            normalized_text=normalize_text(merged_text),
            key=hashlib.sha1(normalize_text(key_seed).encode("utf-8")).hexdigest()[:12],
            cell_bboxes=merged_boxes,
        )
    return merged
...
def _scope_table_score(rows: list[StructuredRow], text: str) -> tuple[bool, int, int]:
    ...
    if noise_header_hits >= 2 and scope_header_hits < 3:
        return False, scope_header_hits, noise_header_hits
    is_scope = (
        (scope_header_hits >= 2 and data_scope_rows >= 1 and scope_text_hits >= 2)
        or (scope_header_hits >= 1 and scope_text_hits >= 1 and data_scope_rows >= 3 and model_like_rows >= 2)
    )
    return is_scope, scope_header_hits, noise_header_hits


def _looks_like_bom_block(rows: list[StructuredRow], text: str) -> tuple[bool, int, int]:
    if len(rows) < 2:
        return False, 0, 0
    is_scope_table, scope_header_hits, _noise_header_hits = _scope_table_score(rows, text)
    normalized = normalize_text(text)
    hint_hits = sum(1 for keyword in TABLE_HINT_KEYWORDS if keyword in normalized)
    model_like_rows = sum(
        1
        for row in rows[1:]
        if any(any(char.isalpha() for char in cell) and any(char.isdigit() for char in cell) for cell in row.cells)
    )
    return is_scope_table or (scope_header_hits >= 2 and hint_hits >= 3 and model_like_rows >= 1), scope_header_hits, hint_hits
...
def _classify_block_kind(bbox: BBox, text: str, rows: list[StructuredRow], page_rect: fitz.Rect) -> tuple[str, float]:
    normalized = normalize_text(text)
    ...
    looks_like_bom, scope_header_hits, _hint_hits = _looks_like_bom_block(rows, text)
    ...
    if looks_like_bom:
        bom_confidence = 0.64 + (0.04 * min(scope_header_hits, 4)) + (0.02 * min(len(rows), 8))
        return "bom-like", min(0.96, bom_confidence)
...
def _is_note_block(region: Region) -> bool:
    text = (region.text or "").strip()
    return bool(text) and NOTES_HEADER_RE.match(text) is not None


def _attach_notes_to_bom_regions(
    bom_regions: list[Region],
    text_regions: list[Region],
    page_index: int,
) -> tuple[list[Region], list[Region]]:
    if not bom_regions or not text_regions:
        return bom_regions, text_regions

    remaining_regions: list[Region] = []
    note_regions = [region for region in text_regions if _is_note_block(region)]
    other_regions = [region for region in text_regions if not _is_note_block(region)]

    updated_boms: list[Region] = []
    for bom_index, bom_region in enumerate(bom_regions):
        attached_notes: list[Region] = []
        bom_width = max(1.0, bom_region.bbox[2] - bom_region.bbox[0])
        bom_height = max(1.0, bom_region.bbox[3] - bom_region.bbox[1])
        for note_region in note_regions:
            horizontal_gap = max(
                0.0,
                max(bom_region.bbox[0], note_region.bbox[0]) - min(bom_region.bbox[2], note_region.bbox[2]),
            )
            vertical_gap = max(
                0.0,
                max(bom_region.bbox[1], note_region.bbox[1]) - min(bom_region.bbox[3], note_region.bbox[3]),
            )
            note_is_left = note_region.bbox[2] <= bom_region.bbox[0] and horizontal_gap <= max(110.0, bom_width * 0.18)
            note_is_above = note_region.bbox[3] <= bom_region.bbox[1] and vertical_gap <= max(72.0, bom_height * 0.30)
            vertical_overlap = bbox_overlap_ratio(
                (bom_region.bbox[0], bom_region.bbox[1], bom_region.bbox[2], bom_region.bbox[3]),
                (note_region.bbox[0], note_region.bbox[1], note_region.bbox[2], note_region.bbox[3]),
            ) > 0.0
            horizontally_aligned = (
                min(bom_region.bbox[2], note_region.bbox[2]) - max(bom_region.bbox[0], note_region.bbox[0])
            ) > 0.0
            if (note_is_left and (vertical_overlap or vertical_gap <= 28.0)) or (note_is_above and horizontally_aligned):
                attached_notes.append(note_region)

        if not attached_notes:
            updated_boms.append(bom_region)
            continue

        attached_ids = {region.region_id for region in attached_notes}
        note_boxes = [region.bbox for region in attached_notes]
        note_text = "\n".join(region.text for region in attached_notes if region.text).strip()
        updated_boms.append(
            Region(
                region_id=f"page-{page_index}-bom-compound-{bom_index}",
                page_index=page_index,
                kind="bom-like",
                bbox=bbox_union([bom_region.bbox, *note_boxes]),
                text="\n".join(part for part in [bom_region.text, note_text] if part).strip(),
                normalized_text=normalize_text("\n".join(part for part in [bom_region.text, note_text] if part)),
                confidence=bom_region.confidence,
                source="bom-compound",
                rows=bom_region.rows,
                metadata={
                    **bom_region.metadata,
                    "compound_scope": "bom_with_notes",
                    "attached_note_region_ids": sorted(attached_ids),
                    "attached_note_count": len(attached_notes),
                    "table_bbox": bom_region.bbox,
                },
            )
        )
        note_regions = [region for region in note_regions if region.region_id not in attached_ids]

    remaining_regions.extend(other_regions)
    remaining_regions.extend(note_regions)
    return updated_boms, remaining_regions
...
def _text_regions(page: fitz.Page, blocks: list[tuple], words: list[tuple], excluded_regions: list[Region], page_index: int) -> list[Region]:
    ...
    for block_index, block in enumerate(blocks):
        ...
        if any(bbox_contains(table_box, bbox) for table_box in table_boxes):
            continue
        rows = _rows_in_bbox(words, bbox)
        kind, confidence = _classify_block_kind(bbox, text, rows, page.rect)
        if kind != "bom-like":
            rows = []
...
def ocr_page_words(page: fitz.Page) -> tuple[list[tuple], bool, str | None]:
    if pytesseract is None:
        return [], False, "pytesseract not installed"
    try:
        pix = page.get_pixmap(dpi=150, alpha=False)
        ...
        x_scale = float(page.rect.width) / max(pix.width, 1)
        y_scale = float(page.rect.height) / max(pix.height, 1)
        words: list[tuple] = []
        for index, raw_text in enumerate(data.get("text", [])):
            text = " ".join(str(raw_text).split())
            confidence = str(data.get("conf", [""])[index]).strip()
            if not text:
                continue
            try:
                if confidence and float(confidence) < 20:
                    continue
            except ValueError:
                pass
            left = float(data["left"][index]) * x_scale
            top = float(data["top"][index]) * y_scale
    ...
    *,
    ...
    if len(native_text.strip()) < 20:
        ...
        if ocr_text.strip():
            ...
            ocr_words, words_attempted, words_error = ocr_page_words(page)
            ocr_attempted = ocr_attempted or words_attempted
            if ocr_words:
                analysis_words = ocr_words
            elif words_error:
                warnings.append(f"OCR word extraction unavailable or failed: {words_error}")
        elif error:
            warnings.append(f"OCR unavailable or failed: {error}")

    table_regions = _table_regions(page, analysis_words, page_index, analysis_text, sheet_family=sheet_family)
    _check_cancel(cancel_requested)
    bom_word_regions = _top_right_bom_regions(page, analysis_words, table_regions, page_index)
    _check_cancel(cancel_requested)
    text_regions = _text_regions(page, blocks, analysis_words, [*table_regions, *bom_word_regions], page_index)
    bom_regions, text_regions = _attach_notes_to_bom_regions([*table_regions, *bom_word_regions], text_regions, page_index)
    ...
    return PageAnalysis(
        ...
        regions=[*bom_regions, *text_regions],
